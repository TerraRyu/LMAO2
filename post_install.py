...
def check_powershell():
    try:
        subprocess.run(["powershell", "-Command", "$PSVersionTable.PSVersion"], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        return True
    except (subprocess.CalledProcessError, FileNotFoundError):
        return False
...
def main():
    ...
    additional_requirements = [
        ...
        "repos/spiderfoot/requirements.txt",
        "repos/CloudFail/requirements.txt",
        "repos/AWSBucketDump/requirements.txt",
        "repos/GCPBucketBrute/requirements.txt"
    ]
    ...
    # Check PowerShell for MicroBurst
    if not check_powershell():
        print("PowerShell is not available. MicroBurst (Azure enumeration) may not work properly.")
        print("Please ensure PowerShell is installed and accessible from the command line.")
